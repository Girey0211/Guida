...
### 5.4. 화면 전환 자동화 `Phase 1`

- 앱 켜져 있는 동안 백그라운드 게임 화면 모니터링
...
| 화면 | 진입 조건 | 이탈 조건 |
|---|---|---|
| **기본화면** | 앱 실행 시 기본 / 플레이화면 뒤로가기 | 거던 탐사 시작 감지 시 자동 전환 |
...
- 현재 층 및 다음 예상 이벤트 선택지 가이드
- `recommended_choice_id` 기준 최적 선택지 Amber 하이라이트
- 판정 죄종 / 임계값 표시
