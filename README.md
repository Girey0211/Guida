...
- **Backend Server**
  ...
  - UUID 기반 중복 추천 방지 (추천은 패치 버전당 1회, 플레이 수는 거던 클리어 시마다 누적)
...
- 추천과 달리 **중복 제한 없이** 해당 루트로 거던을 클리어할 때마다 플레이 수 +1
- 추천수와 마찬가지로 **패치 버전 단위로 집계**
...
[ 유저: 해당 루트로 거던 클리어 ]
        │
        ▼
[ 서버: { route_code, patch_version } 수신 ]
        │
        ▼
[ 중복 제한 없이 플레이 수 +1 반영 (패치 버전 단위 카운터) ]
```

### 7.6. UI 이미지 지연 로딩(Lazy Loading) 및 로컬 캐싱
