...
>
> *"Guida(가이다)"는 단테의 신곡에서 베르길리우스가 단테의 안내자(길잡이)로 불리던 이탈리아어 단어입니다.*
...
│   │   └── playStore.ts          # 플레이 세션 상태 (진행 중 데이터)
│   │
│   ├── hooks/                    # 커스텀 훅
...
### 5.1. 🎯 거울 던전 실시간 선택지 가이드 (Mirror Dungeon Guide)
`Phase 1`
...
### 5.2. 🔗 거던 루트 익명 공유 허브 (Route Sharing Hub)
`Phase 1`
...
### 5.3. 📊 인게임 실시간 보상 추적 (Real-time Reward Tracking)
`Phase 2 — OCR 베타`
...
### 5.4. 🖥️ 화면 전환 자동화 (Screen Auto-transition)
`Phase 1`

- 앱이 켜져 있는 동안 **백그라운드에서 게임 화면을 지속적으로 모니터링**
- 거던 탐사 시작 화면 감지 시 기본화면 → 플레이화면으로 **자동 전환**
- 탐사 종료(강제 종료 버튼 클릭 또는 게임 내 탐사 완료) 감지 시 플레이화면 → 기본화면으로 복귀
...
### 7.2. 루트 업로드 및 코드 발급 (Phase 1 — 자기 신고 방식)
...
[ UI에서 특정 이미지(예: 'greg_01.webp') 요청 ]
              ...
              │         └─► YES: 로컬 저장소에서 즉시 로드 ──► 화면 표시
              │
              └─► [ 조건 B ] 로컬 캐시 폴더에 파일이 없는가? (최초 조회 시)
                        │
                        ├──► [ 서버 정상 ] CDN에 이미지 다운로드 요청
                        │         ├─► 1. 로컬 캐시 폴더에 파일 저장
                        │         └─► 2. 화면에 이미지 표시
                        │
                        └──► [ 서버 다운 ] 다운로드 실패
                                  └─► 앱 내장 기본 대체 이미지 또는 텍스트 타이틀로 대체
```
...
### 8.2. 로컬 — `my_routes.json` (루트 1개 예시)
...
> 📌 `stats` 객체는 패치 버전을 키로 사용하여 버전별 추천수 및 플레이 수를 독립적으로 관리합니다.

### 8.4. 로컬 — 플레이 세션 상태 (`playStore`)

플레이 세션 중 진행 데이터는 `playStore`(Zustand)에 메모리로 유지되며, 세션 종료 시 `my_routes.json`에 병합 저장됩니다.
...
> 📌 `route_switched_at_floor`: 플레이 중 루트를 변경한 경우 해당 층 번호를 기록합니다. 이전 층의 획득 데이터는 보존되며, 변경 시점 이후의 가이드만 새 루트 기준으로 업데이트됩니다.
...
### 9.2. 개발 제약 및 예외 처리 가이드라인

#### 🚫 No Memory Touch

`OpenProcess` 등 게임 프로세스에 직접 관여하는 시스템 API 호출을 **금지**하여 안티치트 오탐지를 원천 차단합니다.
...
기본화면은 사이드 네비게이션 또는 상단 탭으로 아래 세 섹션을 전환합니다.
...
- 필터 패널 (섹션 6 참조) + 루트 카드 리스트
- 카드 클릭 시 상세 모달: 루트 정보 / 추천수 / 플레이수 / 추천 버튼 / 내 루트로 저장 버튼

#### 섹션 C — 설정 (Settings)

- 오버레이 투명도 슬라이더
- 테마 설정 (다크 고정, 향후 확장 고려)
- 앱 버전 정보
...
뒤로가기로 기본화면에 진입한 경우, 우측 상단에 상시 표시됩니다.
...
- 배너 클릭 시 플레이화면으로 즉시 복귀
- 탐사 종료는 플레이화면에서만 가능하며, 기본화면에서는 접근 불가
...
| 요소 | 동작 |
|---|---|
| **← 뒤로가기** | 기본화면으로 이동. 탐사 세션은 유지되며 복귀 배너 표시 |
| **루트 선택 드롭다운** | 내 루트 목록에서 실시간 변경 가능 (변경 즉시 가이드 업데이트) |
| **탐사 종료 버튼** | 확인 다이얼로그 후 세션 종료 → 기본화면으로 이동 |

#### 탭 1 — 에고기프트 (기본 탭)

- 루트 목표 에고기프트 전체 리스트
- **미획득 기프트:** 밝게 표시, 리스트 상단
- **획득 완료 기프트:** 어둡게 처리(opacity 감소) + 리스트 하단으로 이동
- Phase 1: 카드 탭으로 획득 여부 수동 토글
- Phase 2: OCR 자동 반영

#### 탭 2 — 선택지

- 현재 층 및 다음 예상 이벤트의 선택지 가이드
- 루트 목표 재화 기준 최적 선택지 Amber 색상으로 하이라이트

#### 탭 3 — 팩

- 루트상 방문해야 할 팩 전체 목록
- **팩 한정 에고기프트 보유 팩:** 상단 고정 + 강조 표시 (Amber 배지)
- **미방문 팩:** 밝게 표시, 상단
- **방문 완료 팩:** 어둡게 처리 + 하단으로 이동 (에고기프트 탭과 동일 UX)
...
플레이 중 루트를 변경하면 다음 규칙이 적용됩니다.

- 변경 전까지 유저가 토글한 **획득/방문 완료 데이터는 세션에 그대로 보존**
- 새 루트의 목표 리스트와 기존 완료 데이터를 대조하여 **겹치는 항목은 완료 상태로 표시**
- 새 루트에만 있는 미완료 항목은 **상단에 추가**
...
**Charcoal Black** 을 중심으로 한 다크 모드 전용 팔레트입니다. 단테의 신곡에서 베르길리우스가 어두운 지옥과 연옥을 안내하듯, 차갑고 묵직한 어둠 속에서 Amber의 등불이 길을 비추는 시각적 서사를 지향합니다.
...
| `color-accent` | `#E67E22` | 활성 상태, CTA 버튼, 하이라이트 (Flame Amber) |
| `color-text-primary` | `#E1E4E6` | 본문, 헤드라인 (Ash White) |
| `color-text-muted` | `#90969D` | 힌트 텍스트, 부제목, 비활성 아이콘 (Muted Slate) |

> 📌 Muted Slate는 원안 `#888E95`에서 `#90969D`으로 조정되었습니다.
> Card Surface(`#25282C`) 위에서 WCAG AA 기준(4.5:1)을 충족하지 못하는 것을 검증 후 수정한 수치입니다. (4.48 → 4.96)
...
**스플래시 화면**
`color-bg-base` 배경 정중앙에 `color-text-primary` 또는 `color-brand`로 표현된 로고를 심플하게 연출합니다.

**네비게이션 바**
`color-brand` 배경을 적용하고, 현재 활성화된 메뉴 아이콘에만 `color-accent`를 점등하듯 표시합니다.

**버튼**
일반 보조 버튼은 `color-bg-surface` 계열로 배경과 자연스럽게 어우러지게 하고, 핵심 CTA(탐사 종료, 공유하기, 루트 저장 등)에만 `color-accent`를 적용하여 시선을 유도합니다.

**에고기프트 / 팩 획득 상태**
미완료 항목: `color-text-primary` 기준 정상 밝기
완료 항목: opacity 0.35 적용 + 리스트 하단 배치

**선택지 하이라이트**
최적 선택지 좌측 또는 배경에 `color-accent` 보더 또는 배지로 표시
